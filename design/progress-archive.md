...
| 19e | Community: Leaderboard | `Emmab` | DONE ✅ | Module management: multi-point-type (EXP/GEM) recurring leaderboards, no extra incentives + sidebar sub-menu |
| 19f | Community: LB Sprint | `FO9JR` | DONE ✅ | Module management: time-bounded leaderboard sprints with point type badges + non-points incentives (NFT/Token/WL) + sidebar sub-menu |
...
| 19j | Community: Badges | `ezzZ7` | DONE ✅ | Module management: badge types with criteria/rarity/earners + sidebar sub-menu |
| 19k | Community: Access Rules | `PPto3` | DONE ✅ | Settings: member gating rules (wallet age/token holding/identity) + sidebar sub-menu |
| 19l | Community: Homepage Editor | `sQWu1` | DONE ✅ | Settings: C-end homepage customization (sections/theme/visibility) + sidebar sub-menu |
...
| C3 | C-End Leaderboard | `KmdSd` | DONE ✅ | Multi-point-type (EXP/GEM) recurring leaderboard, point type selector, period filters, podium top 3, rankings table |
| C4 | C-End LB Sprint Tab | `y5fUZ` | DONE ✅ | Time-bounded leaderboard sprint with point type badges, NFT/Token incentives, sprint tasks (earn EXP), reward tiers, past LB sprints |
...
| **P0e** | C-End engagement redesign: C01 action engine + C07 User Center + C08 Invite Center + C09 Activity Feed | 3 new + 1 redesign | DONE ✅ |
| **P0f** | Leaderboard vs LB Sprint distinction: multi-point-type leaderboards + time-bounded sprints with incentives | 0 (updates) | DONE ✅ |
| **P0g** | 4-System Module Architecture: Wizard Step 2 redesign + sidebar section headers + Badges/Access Rules/Homepage Editor pages | 3 new + updates | DONE ✅ |
...
### P0e — C-End Engagement Redesign

Strategic redesign of C-End user experience to maximize AARRR engagement:

**Analysis**: Identified 5 critical gaps in C-End:
1. No user identity/profile (users are anonymous)
2. Homepage is broadcast board, not action engine
3. AARRR Referral completely missing
4. Rewards too predictable (no variable rewards)
5. No social presence despite being "Community"

**Changes**:
1. **C01 Community Home (`vJVhd`) redesigned**: Transformed from broadcast board to action engine — Personal Action Card (level/rank/stats/daily bonus), Urgent Actions row (sprint countdown/lucky wheel/leaderboard gap), DayChain streak, task sections, Invite Friends banner, Community Pulse with live stats.
2. **C07 User Center (`nvQuK`) — NEW**: Profile card (avatar/wallet/level/join date), 4 stat cards, 6 achievement badges (earned + locked), activity history timeline (4 items with color-coded types), referral stats (3 cards + invite CTA).
3. **C08 Invite Center (`wSSD5`) — NEW**: Hero with referral link + copy button, 3 share buttons (Twitter/Telegram/Discord), 3 reward tiers (Starter/Ambassador/Legend), Top Inviters leaderboard with "You" highlight.
4. **C09 Activity Feed (`5Nhkz`) — NEW**: Live community feed with green indicator, 5 filter chips, 6 activity items (color-coded by type: task/level-up/invite/wheel/milestone/shop), 3 Trending Now cards.

**Docs updated**: pages.md, layout.md, website_frontend_requirements.md (C07-C09 page codes, nav routing, APIs, dev priority P15).
**Total page codes**: 79 (M01-M14 + B01-B48 + B31a-B31h + C01-C09).

### P0f — Leaderboard vs LB Sprint Distinction

Clarified the conceptual distinction between Leaderboard and Sprint across all B-end and C-end pages:
...
- **Leaderboard Sprint (LB Sprint)** = time-bounded competition with start/end dates, based on custom point types, **with non-points incentives** (NFT/Token/WL Spot).
- Community supports defining multiple custom point types and creating leaderboards/LB Sprints per point type.

**B-end changes**:
- B31d Leaderboard (`Emmab`): Updated to multi-point-type model — stats show "Point Types Tracked", table shows "Point Type" and "Reset Cycle" columns, 3 rows (EXP Weekly / GEM Monthly / Referral Pts All-time)
- B31e Sprint → LB Sprint (`FO9JR`): Renamed, stats show "Incentives Distributed", table shows "Point Type" badges (EXP blue / GEM purple) and "Incentive" column (NFT Airdrop / 500 USDT Pool / WL Spot × 10)
- All 9 community module sidebar sub-menus: "Sprint" → "LB Sprint"

**C-end changes**:
- C03 Leaderboard (`KmdSd`): Title → "EXP Leaderboard", added Point Type selector (EXP/GEM pills), period filters updated, "Tasks Done" column, points display with unit (e.g., "1,250 EXP")
- C04 Sprint → LB Sprint (`y5fUZ`): Title → "Leaderboard Sprint", sprint card shows "March EXP Sprint — NFT Airdrop", tasks labeled "Sprint Tasks (earn EXP)", reward tiers with OG NFT, past sprints with point type labels
- All 9 C-end nav tabs: "Sprint" → "LB Sprint"
- C01 urgent actions and community pulse: Sprint references → LB Sprint
- C09 trending: "Sprint Rush" → "LB Sprint Rush"

**Docs updated**: website_frontend_requirements.md (C03/C04/B31d/B31e descriptions, nav routing, API endpoints /sprint→/lb-sprint, conceptual distinction note).

### P0g — 4-System Module Architecture

Reorganized Community's 9 flat modules into 4 logical systems + 2 infrastructure settings. Created 3 new pages.
...
**Infrastructure (not modules)**: Access Rules, Homepage Editor — appear under SETTINGS in sidebar, not in Configure Systems wizard.

**Design changes**:
- **Wizard Step 2 (`8NeyG`)**: 9 flat toggles → 4 system cards in 2×2 grid. Title: "Configure Systems". Each card has system name, icon, description, grouped sub-module toggles. Core modules auto-enabled (★). Summary panel shows "2 systems · 4 modules".
- **Wizard Step 3 (`qknQZ`)**: Review card updated from "Enabled Modules" to "Enabled Systems" with system-grouped format. Stepper label updated.
- **All 8 community module sidebars**: Added 5 section headers (TASKS/POINTS/CAMPAIGNS/REWARDS/SETTINGS) + 3 new items (Badges `military_tech`, Access Rules `shield`, Homepage Editor `palette`).
- **Badges page (`ezzZ7`) — NEW**: Management page for badge types. Stats: Total Badge Types (6) / Badges Earned (3,847) / Unique Earners (1,240) / Avg per User (3.1). Table: Badge Name / Status / Criteria / Earned By / Rarity / Created. 3 rows: 7-Day Streak, First Quest Complete, Top 10 Leaderboard.
- **Access Rules page (`PPto3`) — NEW**: Community member gating rules. Stats: Active Rules (3) / Users Blocked (42) / Whitelist Entries (156) / Pass Rate (94.2%). Table: Rule Name / Type / Criteria / Blocked / Status / Created. Rule types: Wallet (blue), Token (purple), Identity (green). 3 rows: Min Wallet Age, Token Holdings, Gitcoin Passport.
- **Homepage Editor page (`sQWu1`) — NEW**: C-end homepage customization. Stats: Theme Color (Amber) / Active Sections (5/8) / Last Published (2h ago) / Visitors Today (342). Table: Section / Type / Content / Order / Visibility / Actions. Section types: Banner (amber), Content (blue), Widget (green). 3 rows: Hero Banner, Quest Highlights, Leaderboard Preview.

**Page codes**: B31i (Badges), B49 (Access Rules), B50 (Homepage Editor).
**Total page codes**: 82 (M01-M14 + B01-B50 + B31a-B31i + C01-C09).
...
## Archived from progress.md (2026-03-07)

> Content below was in `design/progress.md` before streamlining. Moved here for reference.

### Previous progress.md Content

**Current Phase** was listed as "Dark Theme Restyle" with the following details:
...
Execution phases P1-P14 + P0-P0j + OB + P3 + P4: see earlier sections of this archive.

#### Key Design Decisions (from progress.md)
...
## Archived from todo.md (2026-03-07)

> Content below was in `design/todo.md` before streamlining. Moved here for reference.
