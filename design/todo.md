# Design TODO

> Active design tasks for `design/pencil-new.pen`
> Completed items tracked in `design/progress.md`

---

## 🟡 P1 — Design Review Remaining Issues (5 structural P1s)

From design review (`design/review-issues.md`), 5 structural items deferred during fix rounds:

| # | Page | Issue | Notes |
|---|------|-------|-------|
| 26 | Community Wizard `Gzpeu` | Missing live preview panel | Wizard was redesigned to 3 steps; preview may no longer be needed since Step 3 is Review |
| 28 | Boost Hub Management `8gT3V` | Missing conversion funnel chart | Could add as a secondary visualization below data table |
| 37 | WL Page Builder Editor `sGDcq` | Canvas area too small | Editor layout is 2-column; canvas could be wider |
| 38 | WL Brand Settings `gS64G` | Missing live preview | Brand settings changes should show preview of C-end appearance |
| 40 | WL Embed Options `RgCVQ` | Overlap with WL Wizard flow | Embed Options is a sub-page; may duplicate some wizard content |

---

## 🟡 P2 — Unified Pricing Page Polish

Page `HO2Ny` is functional but may need:
- Tab switching states for Quest/Community/White Label
- Mobile responsive considerations
- Final copy review

---

## 🟢 P3 — Remaining Frontend Handoff Items

- [ ] Design B48 Dev Kit Page (`taskon.xyz/devkit/{id}`) — standalone developer integration page
- [ ] Verify all canvas annotations on non-P0d pages are still accurate

---

## Recently Completed (see `progress.md` for details)

- ✅ **P0**: Community Empty State redesign (3 retention strategies) + Wizard (3-step)
- ✅ **P0b**: Getting Started Checklist redesign (Community 5-step + WL 6-step)
- ✅ **P0c**: WL expandable sidebar sub-menu (8 pages)
- ✅ **P0d**: Frontend handoff audit — page codes corrected, button routing complete, Dev Kit/Promo Kit/Auto-detection specs added, layout.md corrected, 76 total page codes
- ✅ **Canvas reorganization**: All 75+ pages in structured grid (`design/layout.md`)
- ✅ **Orphaned label fix**: 130+ annotation labels repositioned
